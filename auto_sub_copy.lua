local ffi_available = pcall(require, "ffi")
...
if ffi_available then
    ...
    function copy_to_clipboard(text)
        -- Fallback to using os.execute for platforms without ffi
        os.execute(string.format("echo '%s' | xclip -selection clipboard", text:gsub("'", "'\\''")))
    end
end
